...
NWORDS = train(words(open('trainingdata.txt').read()))
...
def correct(word):
    ...
    a = max(candidates, key=NWORDS.get)
    return a
    # print (a)
def test1():
	tot = 0
	corr = 0
	for w, s in tests1.items():
		allWords = s.split(' ')
		for wrds in allWords:
			tot = tot + 1
			# print (wrds, correct(str(wrds)))
			if correct(wrds) == w:
				corr = corr+1
	print ("Percentage correctness test 1 = ", float(corr/tot)*100)

def test2():
    tot = 0
    corr = 0
    for w, s in tests2.items():
        allWords = s.split(' ')
        for wrds in allWords:
            tot = tot + 1
            # print (wrds, correct(str(wrds)))
            if correct(wrds) == w:
                corr = corr+1
    print ("Percentage correctness test 2 = ", float(corr/tot)*100)
test1()
test2()
