...
public:
	...
	string year_of_enrollment;	// for archive table constraint not in future
...
void getStudent(){
    ...
    while(!sfile.eof()){
    	...
    	sfile >> s.fname >> s.lname >> s.dob >> s.phno >> s.sex >> s.address >> s.section >> s.branch >> s.year_of_enrollment >> s.semester;
...
void addStudent(int isEdit, string rno){
	...
	cin >> s.year_of_enrollment;
	if(!formatYOE(s.year_of_enrollment)){
		cout << "Enter in correct format, enter again\n";
		goto enteryoe;
	}
	entersem:
	...
	sfile << s.rollno << " " << s.fname << " " << s.lname << " " << s.dob << " " <<  s.phno << " " << s.sex << " " <<  s.address << " " <<  s.section << " " <<  s.branch << " " <<  s.year_of_enrollment << " " <<  s.semester << endl;
	if(!isEdit){
		cout << "Student Added\n";
	}
	else{
...
void editStudent(){
    ...
    for(it = Students.begin(); it != Students.end(); ++it) {
    	if(it->rollno != rno){
    		sfile2 << it->rollno << " " << it->fname << " " << it->lname << " " << it->dob << " " <<  it->phno << " " << it->sex << " " <<  it->address << " " <<  it->section << " " <<  it->branch << " " <<  it->year_of_enrollment << " " <<  it->semester << endl;
    	}
    }
    remove("students.txt");
...
void deleteStudent(){
    ...
    for(it = Students.begin(); it != Students.end(); ++it) {
    	if(it->rollno != rno){
    		...
    		sfileArc << it->rollno << " " << it->fname << " " << it->lname << " " << it->dob << " " <<  it->phno << " " << it->sex << " " <<  it->address << " " <<  it->section << " " <<  it->branch << " " <<  it->year_of_enrollment << " " <<  it->semester << endl;
    	}
    }
    remove("students.txt");
    rename("temp.txt", "students.txt");
...
void viewStudentBySection(){
    ...
    for(it = Students.begin(); it != Students.end(); ++it) {
    	if(it->section == sec){
    		cout << it->rollno << "\t" << it->fname << "\t\t" << it->lname << "\t\t" << it->dob << "\t" <<  it->phno << "\t" << it->sex << "\t" <<  it->address << "\t\t" <<  it->section << "\t\t" <<  it->branch << "\t\t" <<  it->year_of_enrollment << "\t\t" <<  it->semester << "\t\t" <<  "YES" <<endl;
    	}
    }
	for(it = ArcStu.begin(); it != ArcStu.end(); ++it) {
    	if(it->section == sec){
    		cout << it->rollno << "\t" << it->fname << "\t\t" << it->lname << "\t\t" << it->dob << "\t" <<  it->phno << "\t" << it->sex << "\t" <<  it->address << "\t\t" <<  it->section << "\t\t" <<  it->branch << "\t\t" <<  it->year_of_enrollment << "\t\t" <<  it->semester << "\t\t" <<  "NO" <<endl;
    	}
    } 
}
void viewStudentByBranch(){
...
void addCourse(int isEdit, string cid){
	...
	course<<co.course_id<<" "<<co.name<<" "<<co.credits<<" "<<co.section<<" "<<co.branch<<" "<<co.semester<<endl;
	if(!isEdit)
...
void editCourse(){
	...
	for(int i = 0; i < courseVector.size(); i++){
		if(courseVector[i].course_id != cid){
			edCourse<<courseVector[i].course_id<<" "<<courseVector[i].name<<" "<<courseVector[i].credits<<" "<<courseVector[i].section<<" "<<courseVector[i].branch<<" "<<courseVector[i].semester<<endl;
		}
	}
	remove("Courses.txt");
...
void viewCourseBySection(){
	...
	int k = 0;
	cout<<"S.No\tCourse Id\tCourse Name\tCredits\t\tSection\t\tBranch\t\tSemester"<<endl;
	for(int i = 0; i < courseVector.size(); i++){
		if(courseVector[i].section == sec){
			k++;
			cout<<k<<".\t"<<courseVector[i].course_id<<"\t\t"<<courseVector[i].name<<"\t\t"<<courseVector[i].credits<<"\t\t"<<courseVector[i].section<<"\t\t"<<courseVector[i].branch<<"\t\t"<<courseVector[i].semester<<endl;
		}
	}
}
void viewCourseByBranch(){
...
void listCourses(){
    ...
    if(!sem){
    	...
    	for(int i = 0; i < courseVector.size(); i++){
    		if(courseVector[i].semester == sem && courseVector[i].branch == bra && courseVector[i].section == sec){
    			k++;
    			cout<<k<<". "<<" "<<courseVector[i].course_id<<"\t"<<courseVector[i].name<<endl;
    		}
    	}
    }
    if(k == 0)
